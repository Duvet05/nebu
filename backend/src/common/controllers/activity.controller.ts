...
export class ActivityController {
  ...
  @Post('migrate')
  @ApiOperation({
    summary: 'Migrate activities from local user ID to authenticated user ID',
    description:
      'Migrates all activities from a temporary local UUID (generated by the app before authentication) ' +
      'to the real authenticated user ID after registration or login. This operation is atomic and uses transactions.',
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Activities migrated successfully',
    type: MigrateActivitiesResponseDto,
  })
  @ApiResponse({
    status: HttpStatus.BAD_REQUEST,
    description: 'Invalid UUIDs provided',
  })
  async migrateActivities(
    @Body() migrateDto: MigrateActivitiesDto,
  ): Promise<MigrateActivitiesResponseDto> {
    return this.activityService.migrateActivities(migrateDto);
  }
